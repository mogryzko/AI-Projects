...
def alphabeta_min_node(board, color, alpha, beta, level, limit):
    ...
    if level == limit:
        ...
        for i in range(0, len(actions)):
            value = min(value, alphabeta_max_node(play_move(board, color, actions[i][0], actions[i][1]), oppositecolor, alpha, beta, level, limit))
            if value <= alpha:
                return value
            beta = min(beta, value)
...
def alphabeta_max_node(board, color, alpha, beta, level, limit):
    ...
    if level == limit:
        ...
        for i in range (0, len(actions)):
            value = max(value, alphabeta_min_node(play_move(board, color, actions[i][0], actions[i][1]), oppositecolor, alpha, beta, level, limit))
            if value >= beta:
                return value
...
def select_move_alphabeta(board, color): 
    ...
    for i in range(0, len(actions)):
        if(play_move(board, color, actions[i][0], actions[i][1]) in board_dict):
            ...
            currentvalue = alphabeta_min_node(play_move(board, color, actions[i][0], actions[i][1]), oppositecolor, alpha, beta, 0, 5)
